...
class CrawlerPage(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        # Target URL
        ttk.Label(left_frame, text="Target URL:").pack(anchor=tk.W)
        self.url_var = tk.StringVar()
        ttk.Entry(left_frame, textvariable=self.url_var, width=40).pack(fill=tk.X, pady=(0, 10))
        ...
        # Max URLs
        ttk.Label(options_frame, text="Max URLs:").pack(anchor=tk.W)
        self.max_urls_var = tk.StringVar(value="500")
    ...
    def create_urls_tab(self):
        ...
        ttk.Button(summary_frame, text="Export URLs", command=self.export_urls).pack(side=tk.RIGHT)
    ...
    def start_crawl(self):
        ...
        self.crawl_thread = threading.Thread(target=self.run_crawl, args=(url,))
        self.crawl_thread.daemon = True
        self.crawl_thread.start()
        
    def run_crawl(self, url: str):
        ...
        try:
            ...
            try:
                results = loop.run_until_complete(
                    self.web_crawler.crawl(url, self.crawl_progress_callback)
                )
    ...
    def import_urls(self, urls: List[str]):
        ...
        if urls:
            self.url_var.set(urls[0])  # Set first URL as target
            if len(urls) > 1:
                messagebox.showinfo("Info", f"Set first URL as target. {len(urls)-1} additional URLs ignored.")
